...
/**
 * @note: in scopes "relative" path is not just
 * @param string
 */
const isRelativePathScope =
  string => string.includes('/') && !isAbsolutePathScope(string);
const isAbsolutePathScope = string => path.isAbsolute(string);
const isPathScope = string => typeof string === 'string' && (
  isRelativePathScope(string) || isAbsolutePathScope(string)
);
